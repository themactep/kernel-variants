...
#endif
#ifdef CONFIG_NV12
		lay_cfg[i].format = LAYER_CFG_FORMAT_NV12;
#endif
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	jzfb->ahb1_gate =clk_get(jzfb->dev,jzfb->ahb1_name);
	if(IS_ERR(jzfb->ahb1_gate))
	{
		ret = dev_err(&pdev->dev,"jzfb ahb1_gate failed!");
		goto err_put_clk;
	}
#endif
...
err_put_clk:
#ifdef CONFIG_SOC_T30
	clk_disable(jzfb->ahb1_gate);
	clk_put(jzfb->ahb1_gate);
#endif
#ifndef CONFIG_FPGA_TEST
	if (jzfb->clk)
		clk_put(jzfb->clk);
